...
async function getTagImages(tag: string, page: number): Promise<FetchResponse> {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://next.quer-durch-asien.de'}/images?tag=${tag}&limit=24&page=${page}`, {
        cache: 'no-store',
    });
    if (!res.ok) {
        throw new Error('Failed to fetch tagged images');
    }
    return res.json();
}
...
export default async function TagPage(props: {
    ...
    try {
        const { images, pagination } = await getTagImages(tag, currentPage);

        const tagNameFormatted = tag.replace(/-/g, ' ');
            ...
            <div className="max-w-6xl mx-auto px-6 py-12 font-sans">
                <header className="mb-12 border-b border-gray-200 pb-6 flex flex-col md:flex-row md:items-end md:justify-between">
                    <div>
                        ...
                        <h1 className="text-4xl font-bold text-gray-900 tracking-tight capitalize">
                            Tag: #{tagNameFormatted}
                        </h1>
                        <p className="mt-2 text-gray-600">
                            Fotos, die mit dem Tag &quot;{tagNameFormatted}&quot; versehen sind
                        </p>
                    </div>
                ...
                {images.length > 0 ? (
                    ...
                    <div className="text-center py-20 bg-gray-50 rounded-lg border border-gray-200">
                        <p className="text-gray-500 italic">Keine Bilder für diesen Tag gefunden.</p>
                    </div>
                )}
